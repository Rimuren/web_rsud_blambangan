<x-layouts::app :title="__('Edit Artikel')">
            ...
            <flux:card class="mb-6 overflow-hidden p-0">
                ...
                <div class="px-3 py-1.5 bg-zinc-50 dark:bg-zinc-900/70 border-b border-zinc-200 dark:border-zinc-800 flex flex-wrap items-center gap-1 toolbar-container">
                    ...
                    <div class="flex items-center gap-0.5 bg-white dark:bg-zinc-800 rounded-md p-0.5 border border-zinc-200 dark:border-zinc-700">
                        <button type="button" title="Tebal"       class="toolbar-btn" data-toolbar="format" data-fmt="bold" onclick="editor.toggleFormat('bold')"><flux:icon name="bold" class="size-3.5" /></button>
                        <button type="button" title="Miring"      class="toolbar-btn" data-toolbar="format" data-fmt="italic" onclick="editor.toggleFormat('italic')"><flux:icon name="italic" class="size-3.5" /></button>
                        <button type="button" title="Garis bawah" class="toolbar-btn" data-toolbar="format" data-fmt="underline" onclick="editor.toggleFormat('underline')"><flux:icon name="underline" class="size-3.5" /></button>
                        <button type="button" title="Coret"       class="toolbar-btn" data-toolbar="format" data-fmt="strike" onclick="editor.toggleFormat('strike')"><flux:icon name="strikethrough" class="size-3.5" /></button>
                    </div>

                    {{-- Heading --}}
                    <div class="flex items-center gap-0.5 bg-white dark:bg-zinc-800 rounded-md p-0.5 border border-zinc-200 dark:border-zinc-700">
                        <button type="button" title="Heading 1" class="toolbar-btn text-[11px] font-bold px-1.5" data-toolbar="header" data-level="1" onclick="editor.toggleHeader(1)">H1</button>
                        <button type="button" title="Heading 2" class="toolbar-btn text-[11px] font-bold px-1.5" data-toolbar="header" data-level="2" onclick="editor.toggleHeader(2)">H2</button>
                        <button type="button" title="Heading 3" class="toolbar-btn text-[11px] font-bold px-1.5" data-toolbar="header" data-level="3" onclick="editor.toggleHeader(3)">H3</button>
                        <button type="button" title="Paragraf"  class="toolbar-btn text-[11px] px-1.5" data-toolbar="header" data-level="false" onclick="editor.toggleHeader(false)">P</button>
                    </div>

                    {{-- List --}}
                    <div class="flex items-center gap-0.5 bg-white dark:bg-zinc-800 rounded-md p-0.5 border border-zinc-200 dark:border-zinc-700">
                        <button type="button" title="Daftar Angka" class="toolbar-btn" data-toolbar="list" data-list-type="ordered" onclick="editor.toggleList('ordered')"><flux:icon name="numbered-list" class="size-3.5" /></button>
                        <button type="button" title="Daftar Bullet" class="toolbar-btn" data-toolbar="list" data-list-type="bullet" onclick="editor.toggleList('bullet')"><flux:icon name="list-bullet" class="size-3.5" /></button>
                    </div>

                    {{-- Align --}}
                    <div class="flex items-center gap-0.5 bg-white dark:bg-zinc-800 rounded-md p-0.5 border border-zinc-200 dark:border-zinc-700">
                        <button type="button" title="Rata Kiri"   class="toolbar-btn" data-toolbar="align" data-align="" onclick="editor.toggleAlign('')"><flux:icon name="bars-3-bottom-left" class="size-3.5" /></button>
                        <button type="button" title="Rata Tengah" class="toolbar-btn" data-toolbar="align" data-align="center" onclick="editor.toggleAlign('center')"><flux:icon name="bars-3" class="size-3.5" /></button>
                        <button type="button" title="Rata Kanan"  class="toolbar-btn" data-toolbar="align" data-align="right" onclick="editor.toggleAlign('right')"><flux:icon name="bars-3-bottom-right" class="size-3.5" /></button>
                    </div>

                    {{-- Media --}}
                    <div class="flex items-center gap-0.5 bg-white dark:bg-zinc-800 rounded-md p-0.5 border border-zinc-200 dark:border-zinc-700">
                        <button type="button" title="Tambah Link"          class="toolbar-btn" onclick="editor.openLinkModal()"><flux:icon name="link" class="size-3.5" /></button>
                        <button type="button" title="Tambah Gambar URL"    class="toolbar-btn" onclick="editor.openImageModal()"><flux:icon name="photo" class="size-3.5" /></button>
                        <button type="button" title="Upload Gambar"        class="toolbar-btn" onclick="editor.uploadImage()"><flux:icon name="arrow-up-tray" class="size-3.5" /></button>
                        <button type="button" title="Hapus Gambar Terpilih" class="toolbar-btn text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20" onclick="editor.deleteSelectedImage()"><flux:icon name="trash" class="size-3.5" /></button>
                    </div>
                    ...
                    {{-- Remove Format --}}
                    <div class="ml-auto">
                        <button type="button" title="Hapus Format" class="toolbar-btn !text-zinc-400 hover:!text-red-500" onclick="editor.removeFormat()"><flux:icon name="x-mark" class="size-3.5" /></button>
                    </div>
                </div>
    ...
    <script>
        const editor = new ArticleEditor({
            ...
            oldContent     : @json(old('konten', $artikel->konten)),
        });
        editor.init();
    </script>
<<<<<<< HEAD
