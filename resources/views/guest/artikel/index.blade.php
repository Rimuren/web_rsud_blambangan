...
<div class="min-h-screen py-6 md:py-10 px-4 md:px-7 bg-[#e8f0f7]">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="mb-8 md:mb-10">
            ...
            <h1 class="text-3xl md:text-5xl font-extrabold text-blue-900 leading-tight mb-3">
                Wawasan Kesehatan & <span class="text-orange-500"><br>Saran Ahli</span>
            </h1>
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <p class="text-gray-600 text-sm md:text-base max-w-md leading-relaxed">
                ...
                <div class="flex items-center gap-1.5 text-gray-500 text-xs md:text-sm whitespace-nowrap">
                    <i class="fa-regular fa-circle-check text-gray-500 text-sm"></i>
        ...
        <div class="flex flex-wrap gap-2 mb-8">
            <a href="{{ route('guest.artikel.index') }}" 
               class="flex items-center gap-1.5 px-3 py-1.5 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-semibold {{ !request('kategori') ? 'bg-blue-900 text-white' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50' }} transition">
                <i class="fa-solid fa-border-all text-xs"></i> Semua
            </a>
            @foreach($categories as $cat)
            <a href="{{ route('guest.artikel.index', ['kategori' => $cat->slug]) }}" 
               class="flex items-center gap-1.5 px-3 py-1.5 md:px-4 md:py-2 rounded-full text-xs md:text-sm font-medium {{ request('kategori') == $cat->slug ? 'bg-blue-900 text-white' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50' }} transition">
        ...
        {{-- Kotak Pencarian --}}
        <div class="bg-white rounded-2xl border border-gray-200 px-4 py-3 md:px-5 md:py-4 mb-8">
            <p class="text-sm font-bold text-blue-900 mb-2 md:mb-3">Cari Artikel</p>
            <form method="GET" action="{{ route('guest.artikel.index') }}">
                <div class="relative">
                    <input type="text" name="search" placeholder="Cari artikel..." value="{{ request('search') }}"
                           class="w-full border border-gray-200 rounded-lg px-4 py-2 text-sm text-gray-600 placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-blue-200 pr-10">
        ...
        {{-- Grid Artikel --}}
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6 mb-10">
        ...
        <div class="custom-pagination">
            ...
            @else
                <a href="{{ $articles->previousPageUrl() }}" rel="prev"><i class="fa-solid fa-chevron-left"></i> Previous</a>
            @endif
            ...
            @if ($articles->hasMorePages())
                <a href="{{ $articles->nextPageUrl() }}" rel="next">Next <i class="fa-solid fa-chevron-right"></i></a>
            @else
        ...
        <div class="text-center text-sm text-gray-500 mt-4">
            Menampilkan {{ $articles->firstItem() }} sampai {{ $articles->lastItem() }} dari {{ $articles->total() }} hasil
        </div>
    </div>
</div>
@endsection